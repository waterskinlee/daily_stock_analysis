...
const settingsHelpZhCN: SettingsHelpMap = {
  ...
  'settings.ai_model.LLM_REASONING_EFFORT': {
    title: 'Reasoning Effort 默认值',
    summary: '为 OpenAI 协议模型设置全局 reasoning effort 兜底。',
    usage: '可选 none、minimal、low、medium、high、xhigh、max；留空则使用模型或网关默认值。',
    valueNotes: ['该值不会覆盖模型级映射。', '非 OpenAI 协议请求不会发送此参数。'],
    impact: ['影响 OpenAI 协议模型的推理预算与响应延迟。'],
    notes: ['provider 不支持时请求会按参数恢复机制重试并在当前进程缓存省略策略。'],
  },
  'settings.ai_model.LLM_REASONING_EFFORTS_JSON': {
    title: '按模型 Reasoning Effort',
    summary: '按精确 provider/model route alias 单独覆盖 reasoning effort。',
    usage: '填写 JSON 对象，例如 {"openai/gpt-5.6-sol":"xhigh"}；键必须带 provider 前缀。',
    valueNotes: ['模型级值优先于全局 LLM_REASONING_EFFORT。', '非法键或值会被忽略并回退。'],
    impact: ['影响对应 OpenAI 模型的推理预算，不改变其它模型。'],
    notes: ['仅 protocol=openai 请求使用；DeepSeek、Anthropic、Gemini、Ollama 保持各自 thinking 配置。'],
  },
...
const settingsHelpEnUS: SettingsHelpMap = {
  ...
  'settings.ai_model.LLM_REASONING_EFFORT': {
    ...
    summary: 'Sets the global reasoning-effort fallback for OpenAI-protocol models.',
    usage: 'Use none, minimal, low, medium, high, xhigh, or max; leave empty for the model or gateway default.',
    valueNotes: ['This does not override a per-model mapping.', 'Non-OpenAI protocol requests omit this parameter.'],
    impact: ['Affects reasoning budget and latency for OpenAI-protocol models.'],
    notes: ['If a provider rejects it, request recovery retries once and caches omission for the current route shape.'],
  },
  'settings.ai_model.LLM_REASONING_EFFORTS_JSON': {
    title: 'Reasoning Effort Per Model',
    summary: 'Overrides reasoning effort for exact provider/model route aliases.',
    usage: 'Use a JSON object such as {"openai/gpt-5.6-sol":"xhigh"}; keys must include a provider prefix.',
    valueNotes: ['Per-model values override LLM_REASONING_EFFORT.', 'Invalid keys or values are ignored and fall back.'],
    impact: ['Affects the reasoning budget for matching OpenAI models without changing other routes.'],
    notes: ['Only protocol=openai requests use it; DeepSeek, Anthropic, Gemini, and Ollama keep provider-specific thinking settings.'],
  },
  'settings.ai_model.LLM_PROMPT_CACHE_TELEMETRY_ENABLED': {
