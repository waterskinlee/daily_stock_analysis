...
- 若提示词中注入了「上次分析观察点」section，必须输出 `dashboard.previous_watch_verification` 结构化核对字段：`has_previous=true`，`items` 数组逐条含 condition/status（fulfilled/not_fulfilled/partially_fulfilled/stale）/evidence/impact，`summary` 整体核对结论。
