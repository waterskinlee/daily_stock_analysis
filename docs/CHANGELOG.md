...
- [改进] 飞书云文档的「个股决策仪表盘」固定使用完整（full）格式渲染，不再跟随 `REPORT_TYPE`：消息端可自由选择精简/简洁而文档始终保留数据透视、作战计划等完整小节。
...
- [改进] Web 选股页策略下拉支持按类别与风险筛选，选中策略后展示核心逻辑、关键硬过滤、技术面权重与日 K 数据要求，避免盲选。
...
- [新功能] A股资金面上下文新增东财大宗交易与个股两融明细：结构化汇总近 90 日成交额、折溢价分布及最新融资余额、1/3/5/10 日融资净买入和融券净卖出，并与 Tushare 主力资金流统一写入分析提示词及 Agent `get_capital_flow` 工具；上游异常按子源 fail-open，不阻断技术面与基本面分析。
- [修复] 个股分析链路统一传播强制刷新、批次 `query_id` 与逐股 `trace_id`；报告保存后回写最终诊断快照，并以显式 `history_saved` 与 Agent `success/degraded/failed` 状态驱动 API、历史记录和 Web 诊断，避免缓存未刷新、链路身份断裂、保存失败或降级结果被误报成功。
- [修复] 上次观察点兜底改用本轮价格、量比与主力资金事实逐条核验，复合条件保持保守的部分兑现语义；通知与历史 Markdown 共用同一阶段决策/观察点渲染，并过滤与确定性上下文冲突的数据缺失声明。
- [改进] 定时分析逐股持久化 `completed_count`，并在调度器启动时按 `SCHEDULED_RUN_MAX_AGE_MINUTES` 对账遗留 `running` 记录，避免进程中断后批次永久显示运行中。
