...
- [改进] Agent 模式报告完整性校验由「占位补全」升级为「有界 LLM 修复→占位兜底」：缺失必填字段时先发起一次无状态文本补全调用（复用 `executor.llm_adapter.call_text`，温度 0、60s 超时、最多 `REPORT_INTEGRITY_RETRY` 次），补全 prompt 携带上次观察点 section 原文 + 今日行情/趋势紧凑摘要作为证据接地，避免证据型字段（如 `previous_watch_verification.evidence`）被编造；修复响应严格 merge-only（仅写入缺失路径、绝不覆盖已有字段），解析失败/超时/重检仍缺则落回 `apply_placeholder_fill` 占位；三语本地化摘要标签，新增 16 项 `TestAgentIntegrityRepair` 单测覆盖 merge-only/根归一/超时兜底/parse-error 重试/prompt 接地。
