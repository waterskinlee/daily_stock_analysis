...
> **Behavior Notes:**
> - **A-shares**: Returns aggregated capabilities by `valuation/growth/earnings/institution/shareholder_actions/capital_flow/dragon_tiger/lockup/boards`. `shareholder_actions` uses Tushare/xiaodefa for equity pledges, repurchase progress, and major-shareholder/director increases or decreases; `lockup` (restricted-share unlock calendar, Eastmoney datacenter direct HTTP) and `earnings.consensus_eps` (同花顺 consensus EPS) are free keyless direct feeds. Every sub-source fails open.
