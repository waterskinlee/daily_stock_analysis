...
- Deployment: local `F:\dsa` is development-only (never run servers). Ubuntu 192.168.1.197 hosts test (`~/dsa-test`, port 8001) and prod (`~/daily_stock_analysis`, port 8000, image `daily_stock_analysis:overlay-<commit>`). "Deploy to port 8000" = Ubuntu prod, never localhost. Sync: push origin dev → `~/dsa-clone` → `~/dsa-repo.git` → `~/dsa-test` → prod overlay build. Back up prod `data/` (`stock_analysis.db`, `runtime.env`) to `~/backup/` before each sync. See `AGENTS.md` "Deployment & Environment" and `DEV_WORKFLOW.md`.
