...
- Deployment topology (mirror of AGENTS.md "Deployment & Environment"): local Windows `F:\dsa` is development-only — never run servers here. Both environments live on Ubuntu 192.168.1.197 under the single canonical root `~/dsa-test`: PROD 8000 via `docker/docker-compose.yml` (containers `stock-server`/`stock-analyzer`, real data in `data/`) and TEST 8001 via `docker compose -p dsa-test -f docker/docker-compose.test.yml` (containers `dsa-test-*`, isolated throwaway data in `data-test/`). "Deploy to port 8000" always means the Ubuntu prod env. Sync flow: local commit → push origin dev (+ push to `ssh://ubuntu/home/waterskin/dsa-repo.git dev:dev` when GitHub is unreachable) → `~/dsa-test` fetch + reset to FETCH_HEAD → validate 8001 → promote 8000 (`compose restart server analyzer` for code-only; `up -d --force-recreate` after any env/mount change). The only live runtime config is `~/dsa-test/data/runtime.env` (startup-read; verify via `/api/v1/system/config` after changes) and the only prod DB is `data/stock_analysis.db`. Snapshot env + data to `~/backup/` before recreating containers or touching config. NEVER run compose from `~/daily_stock_analysis` — it is retired and its compose declares the same container names/port as prod. See `DEV_WORKFLOW.md` (gitignored, untracked) for full details.
